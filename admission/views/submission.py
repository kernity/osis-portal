...
from admission.utils import file_reference, applicant_profil
...
VARIABLE_SUBJECT_TO_QUOTA_TXT = 'subject_to_quota_txt'
...
VARIABLE_ADMISSION_PREVIOUS_ACADEMIC_YEAR = 'previous_academic_year'
...
VARIABLE_URL_INFO = 'url_info'
VARIABLE_ADMISSION_URL_INFO = 'url_info'
...
def get_submission_docs_url_info(a_grade_type, a_language):
    if a_language == FRENCH_LANGUAGE_CODE:
        return get_submission_docs_url_fr(a_grade_type)
    else:
        return get_submission_docs_url_en(a_grade_type)
...
def get_title(applicant):
    ...
    if applicant.gender == mdl.applicant.FEMALE:
        if applicant.civil_status != mdl.applicant.MARRIED:
            ...
            return _('miss')
    return "{0}, {1}".format(_('miss'), _('mister'))
...
def message_template_admission_variables(an_applicant, an_application):
    data = {VARIABLE_LAST_NAME: an_applicant.user.last_name,
            ...
            VARIABLE_REFERENCE: file_reference.get_reference(an_application, an_applicant),
            ...
            VARIABLE_SUBJECT_TO_QUOTA_TXT:
                get_subject_to_quota_txt(an_application.offer_year.subject_to_quota),
            ...
            VARIABLE_CLOSING_DATE: get_sending_limit(an_application.submission_date),
            VARIABLE_ADMISSION_URL_INFO: get_submission_docs_url_info(an_application.offer_year.grade_type,
                                                                      an_applicant.language), }
    return data
...
def get_subject_to_quota_txt(subject_to_quota):
    if subject_to_quota is True:
        return "{0}{1}<br>{0}{1}<br>{0}{1}<br>".format(_('subject_to_quota_txt_part_1'),
                                                       1,
                                                       _('subject_to_quota_txt_part_2'),
                                                       2,
                                                       _('subject_to_quota_txt_part_3'),
                                                       3)
    return ""
...
def message_template_inscription_variables(an_applicant, an_application):
    data = {VARIABLE_LAST_NAME: an_applicant.user.last_name,
            ...
            VARIABLE_ORGANIZATION_CITY: '',
            VARIABLE_ORGANIZATION_COUNTRY: '',
            VARIABLE_CLOSING_DATE: applicant_profil.get_limit_sending_date(an_application, an_applicant),
            ...
            VARIABLE_SUBJECT_TO_QUOTA_TXT: get_subject_to_quota_txt(an_application.offer_year.subject_to_quota),
            ...
            VARIABLE_URL_INFO: get_url(an_application.offer_year.grade_type)}
    return data


def get_message_document(an_applicant, an_activity_type):
    curriculum_list = mdl.curriculum.search(an_applicant, an_activity_type, None)
    if curriculum_list.exists():
        return "<li>{0}</li>".format(UNEMPLOYMENT_DOC_NEEDED_TXT)
    return ""


def get_debts_document(an_application):
    if accounting.debts_check(an_application):
        return "<li>{0}</li>".format(_(DEBTS_DOC_NEEDED_TXT))
    return ""
...
def build_messages(curriculum_list):
    if curriculum_list.exists():
        ...
        for cv in curriculum_list:
            msg = msg + "<li>{0} {1}</li>".format(_(OTHER_DOC_NEEDED_TXT), get_academic_year(cv.academic_year))
        return msg
    return ""
            ...
            if secondary_education_exam.admission_exam_type:
                ...
                if offer_admission_exam_type:
                    if secondary_education_exam.admission_exam_type == offer_admission_exam_type.admission_exam_type:
                        return "<li>{0}</li>".format(_(SPECIAL_EXAM_ADMISSION_NEEDED_TXT))
                else:
                    return "<li>{0}</li>".format(_(EXAM_ADMISSION_DOC_NEEDED_TXT))
    return ""
...
def get_message_secondary_diploma_document(an_applicant):
    ...
    if a_secondary_education:
        return "<li>{0} {1}</li>".format(_(SECONDARY_DIPLOMA_NEEDED_TXT),
...
def get_message_capaes_document(an_institutional_grade_type):
    if is_capaes(an_institutional_grade_type):
        return "<li>{0}</li>".format(_(CAPAES_DOC_NEEDED_TXT))
    return ""
...
def text_display(an_applicant, an_application):
    ...
    print(an_application.application_type)
    ...
    ch = Template(template_reference.template).render(Context(data))
    return ch


def get_url(a_grade_type):
    if a_grade_type.url_info:
        return a_grade_type.url_info
    return ""


def get_sending_limit(submission_date):
    if submission_date:
        return (submission_date + timedelta(days=30)).strftime('%d/%m/%Y')
    return ""
